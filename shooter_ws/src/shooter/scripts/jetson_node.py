...
def callback(data):
	...
	#Arcade Drive
	#Mapped for Logitech Controller; not necessarily the SHIELD keys
	ldrive,rdrive = arcadeDrive(float(data.axes[1]),float(-data.axes[3]))
	...
	lshoot = 1 if float(data.axes[2]) < 0.0 else 0
	rshoot = 1 if float(data.axes[5]) < 0.0 else 0
	lshoot = lshoot and not old_lshoot
	rshoot = rshoot and not old_rshoot
	
	lbumper = int(data.buttons[4])
	rbumper = int(data.buttons[5])

	lreload = int(data.buttons[2])
	lreload = lreload and not old_lreload
	rreload = int(data.buttons[1])
	rreload = rreload and not old_rreload

	pivotForward = int(data.buttons[3])
	pivotBackward = int(data.buttons[0])
